...
test_data_zip = [('us', '90210', 'Beverly Hills'),
                 ('it', '50123', 'Firenze'), ('ca', 'Y1A', 'Whitehorse')]
...
def test_get_data_for_zip_check_name(country_code, zip_code, place_name):
...
def read_data_from_csv():
    test_data_places_from_csv = []
    with open("exercises/test_data_places.csv", newline="") as csvfile:
        data = csv.reader(csvfile, delimiter=",")
        for row in data:
            test_data_places_from_csv.append(row)
    return test_data_places_from_csv
